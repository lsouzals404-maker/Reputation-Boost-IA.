...
// 🔥 URL BASE CENTRAL (IMPORTANTE)
const appUrl = readEnv("APP_URL", `http://localhost:${port || 3000}`);
...
export const config = {
  ...
  sessionSecret: readEnv(
    "SESSION_SECRET",
    "reputation-boost-ia-dev-secret"
  ),
  ...
  // ✅ USA appUrl (não duplicado)
  stripeReturnUrl: readEnv(
    "STRIPE_RETURN_URL",
    `${appUrl}/#billing`
  ),

  asaasApiKey: readEnv("ASAAS_API_KEY"),
  asaasBaseUrl: readEnv(
    "ASAAS_BASE_URL",
